    ...
    data.forEach((element: any) => {
      if (Array.isArray(element)) {
        element.forEach((subElement: FormElement) => {
          if (subElement.required && subElement.type!=='email') {
            schema[subElement.id] = Yup.string().required("Required");
          }else if(subElement.required && subElement.type ==='email') {
            schema[subElement.id] = Yup.string().required("Required").email('Invalid email');
          }
        });
      } else if (element.required && element.type !=='email') {
        schema[element.id] = Yup.string().required("Required");
      }else if (element.required && element.type ==='email') {
        schema[element.id] = Yup.string().required("Required").email('Invalid email');
      }
    });
